      ...
      {Object.entries(rasedSummary).map(([saf, fasels]) => (
        <React.Fragment key={saf}>
          {Object.entries(fasels).map(([fasel, periodsData]) => {
            ...
            if (!targetPeriods.some(p => periodsData[p])) return null;

            return (
              <div key={`${saf}-${fasel}`} className="print-page-class-stats keep-together">
                <div className="bg-white border p-8 rounded-[2.5rem] shadow-lg text-center mb-8 print:border-black print-card">
                  <h3 className="text-2xl font-black">إحصائيات رصد الفصل: {saf} - {fasel}</h3>
                  <p className="text-slate-500 font-bold mt-1">صفحة إحصائيات رصد الفصل</p>
                </div>
