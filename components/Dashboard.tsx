  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="print-avoid-break">
        <div className="bg-white rounded-[2.5rem] p-10 md:p-14 shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center relative overflow-hidden print:p-8 print:rounded-3xl print-card">
          <div className="absolute top-0 right-0 w-1/3 h-full bg-blue-50/50 -skew-x-12 translate-x-1/2 no-print"></div>
          
          <div className="relative z-10 text-center md:text-right space-y-6 print:space-y-2">
            <div>
              <span className="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider mb-4 shadow-lg shadow-blue-200 print:mb-1 print:bg-blue-700">
                <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse print:hidden"></span>
                تقرير الحالة الراهنة
              </span>
              <h2 className="text-4xl md:text-5xl font-black text-slate-950 tracking-tight leading-tight print:text-2xl">إنجاز رصد المدرسة</h2>
              <p className="text-slate-500 font-bold text-lg mt-2 print:text-sm print:mt-0">{periodLabel}</p>
            </div>
          ...
          <div className="relative z-10 mt-12 md:mt-0 text-center flex flex-col items-center print:mt-0">
            <div className="relative inline-flex items-center justify-center">
              <svg className="w-48 h-48 md:w-56 md:h-56 transform -rotate-90 print:w-32 print:h-32">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-5xl md:text-6xl font-black text-slate-950 print:text-2xl">{stats.percentage}%</span>
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1 print:text-[7px]">نسبة الاكتمال</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mt-6 print:gap-4 print:mt-4">
          <StatCard title="الطلاب" value={stats.studentCount} color="blue" />
          <StatCard title="المواد" value={stats.subjectCount} color="emerald" />
          <StatCard title="الفصول" value={stats.classesCount} color="amber" />
          <StatCard title="المعلمون" value={stats.teacherCount || "---"} color="indigo" />
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 print:block">
        {/* صفحة 2 عند الطباعة: ترتيب الفصول */}
        <div className="lg:col-span-2 bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm print:p-8 print:mt-10 print-page-break print-card">
          <div className="flex justify-between items-center mb-10 print:mb-4">
            <h3 className="text-xl font-black text-slate-950 flex items-center gap-3 print:text-base">
          ...
          <div className="space-y-7 print:space-y-3">
            {stats.classesList.slice(0, 10).map((cls, idx) => (
              <div key={idx} className="group">
                <div className="flex justify-between items-center mb-2.5 px-1 print:mb-1">
                  <div className="flex items-center gap-4 print:gap-2">
                    <span className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-black bg-slate-50 text-slate-400 border border-slate-200 group-hover:bg-blue-600 group-hover:text-white transition-all print:w-6 print:h-6 print:text-[9px]">{idx + 1}</span>
                    <span className="font-bold text-slate-800 text-sm print:text-xs">{cls.name}</span>
                  </div>
                  <span className={`text-xs font-black print:text-[10px] ${cls.percentage === 100 ? 'text-emerald-600' : 'text-slate-950'}`}>{cls.percentage}%</span>
                </div>
                <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden border border-slate-50 print:h-1.5">
        ...
        {/* تفاصيل الرصد - لا تظهر في الصفحة المستقلة للترتيب */}
...
const StatCard = ({ title, value, color }: any) => {
  ...
  return (
    <div className="bg-white p-7 rounded-[2rem] border border-slate-200 shadow-sm flex items-center gap-6 transition-all group print:p-3 print:rounded-xl print:gap-3">
      <div className={`p-4 rounded-2xl ${colors[color]} border transition-transform group-hover:scale-110 print:p-2 print:rounded-lg`}>
        <div className="w-7 h-7 flex items-center justify-center print:w-4 print:h-4">✨</div>
      </div>
      <div>
        <span className="block text-slate-400 text-[9px] font-black uppercase tracking-widest mb-1 print:text-[7px] print:mb-0">{title}</span>
        <span className="text-2xl font-black text-slate-950 print:text-base">{value}</span>
      </div>
    </div>
  );
};
